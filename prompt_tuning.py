...
class SaveEveryEpochCallback(TrainerCallback):
    def on_epoch_end(self, args, state, control, model=None, **kwargs):
        epoch = state.epoch
        save_dir = f"{args.output_dir}/checkpoint-epoch-{int(epoch)}"
...
def load_tokenize_data(args):
    ...
    if os.path.exists(args.cache_data):
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.load,local_files_only=True)
        tokenizer.add_tokens(['[PATCH]','[BUG]','[CE]','[FE]','[CONTEXT]','[BUGGY]','[CLASS]','[METHOD]','[RETURN_TYPE]','[VARIABLES]','[Delete]'])
        def preprocess_function(examples):
            ...
            model_inputs = tokenizer(source ,max_length=args.max_input_len, padding="max_length", truncation=True)#
            labels = tokenizer(target,max_length=args.max_output_len, padding="max_length",  truncation=True)#
...
def main(args):
    ...
    if args.data_num != -1:
        train_data = train_data.shuffle(seed=args.seed).select([i for i in range(args.data_num)])
    ...
    peft_config = PromptTuningConfig(
        task_type=TaskType.SEQ_2_SEQ_LM,  # Sequence-to-sequence fine-tuning
        num_virtual_tokens=40,  # Number of soft prompt tokens
        num_layers=4,
        token_dim=model.config.encoder.n_embd,
        num_attention_heads=model.config.encoder.n_head,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-input-len', default=768, type=int)
